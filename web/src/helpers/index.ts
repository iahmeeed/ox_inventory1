...
export const findAvailableSlot2 = (item: Slot, data: ItemData, items: Slot[]) => {
...
export const findAvailableSlot = (item: Slot, data: ItemData, items: Slot[], invtype: string) => {
  if (invtype === 'player') {
    if (!data.stack) return items.slice(9).find((target) => target.name === undefined);

    const stackableSlot = items.slice(9).find((target) => target.name === item.name && isEqual(target.metadata, item.metadata));

    return stackableSlot || items.slice(9).find((target) => target.name === undefined);
  } else{
...
export const getItemUrl = (item: string | SlotWithItem) => {
  ...
  const withExt = (image: string) => {
    // Se já tiver uma extensão, retorna como está
    if (/\.(png|webp|svg)$/i.test(image)) return image;

    // Se não tiver, adicionamos .png (ou troque para .webp se preferir)
    return `${image}.png`;
  };
  ...
  if (isObj) {
    ...
    if (metadata?.image) return withExt(`${imagepath}/${metadata.image}`);
  }

  const itemName = isObj ? (item.name as string) : item;
  const itemData = Items[itemName];

  if (!itemData) return withExt(`${imagepath}/${itemName}`);
  if (itemData.image) return itemData.image;

  itemData.image = withExt(`${imagepath}/${itemName}`);
