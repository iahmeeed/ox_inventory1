...
export const onDrop = (source: DragSource, target?: DropTarget, splitCount?: number) => {
  ...
  if (sourceSlot.metadata?.container !== undefined) {
    ...
    if ((state.rightInventory.id || state.leftInventoryBottom.id)  === sourceSlot.metadata.container)
  ...
  if (target) {
    targetSlot = target.item.slot === -1 
      ? findAvailableSlot2(sourceSlot, sourceData, targetInventory.items) 
  ...
  if (targetSlot.metadata?.container !== undefined && (state.rightInventory.id || state.leftInventoryBottom.id) === targetSlot.metadata.container)
