...
local function ConvertQB()
	...
	local players = MySQL.query.await('SELECT citizenid, inventory, money FROM players')
	...
	Print(('Converting %s QB player inventories to ox_inventory format'):format(total))

	for i = 1, total do
		count += 1
		local inventory, slot = {}, 0

		local items = players[i].inventory and json.decode(players[i].inventory) or {}
		local money = players[i].money and json.decode(players[i].money) or {}

		-- 💰 Convert money (optional but recommended)
		for k, v in pairs(money) do
			if v > 0 and Items(k) then
				slot += 1
				inventory[slot] = {
					slot = slot,
					name = k,
		...
		for _, item in pairs(items) do
			if item and item.name then
				...
				if item.type == 'weapon' then
					...
					metadata.components = metadata.attachments or {}
					metadata.serial = metadata.serial or GenerateSerial()
				end
	...
	Print('QB → ox_inventory conversion completed')
	started = false
end
